...
describe("GLX Contract", function () {
  let GLX: ContractFactory;
  let glx: Contract;
  let admin: Signer;
  let minter: Signer;
  let account1: Signer;
  ...
  beforeEach(async function () {
    ...
    GLX = await ethers.getContractFactory("GLX");
    glx = await GLX.connect(admin).deploy(
  ...
  it("Should have correct initial values", async function () {
    ...
    expect(await glx.symbol()).to.equal("GLX");
    expect(await glx.decimals()).to.equal(18);
  
    // Test total supply
    expect(await glx.totalSupply()).to.equal(0);
  
    // Test admin and minter addresses
    expect(await glx.admin()).to.equal(await admin.getAddress());
    expect(await glx.minter()).to.equal(await minter.getAddress());
  
    // Test stake modifier contract address
    expect(await glx.stakeModifier()).to.equal(ethers.constants.AddressZero);
  });  

  it("Should allow token transfers", async function () {
    // Mint some tokens to account1
    const amountToMint = ethers.utils.parseEther("1000");
    await glx.connect(minter).mint(await account1.getAddress(), amountToMint);
    ...
    const balanceAccount1 = await glx.balanceOf(await account1.getAddress());
    const balanceAccount2 = await glx.balanceOf(await account2.getAddress());
  ...
  it("Should allow token transfers with approved allowances", async function () {
    ...
    const amountToMint = ethers.utils.parseEther("1000");
    await glx.connect(minter).mint(await account1.getAddress(), amountToMint);
    await glx.connect(minter).mint(await account2.getAddress(), amountToMint);
  ...
  it("Should not allow token transfers if sender does not have enough balance", async function () {
    ...
    try {
      ...
      expect(error.message).to.include("GLX::_transferTokens: transfer amount exceeds balance");
    }
  });
  ...
  it("Should emit Transfer event on token transfers", async function () {
    ...
    for (const log of receipt.logs) {
      try {
        const parsedLog = glx.interface.parseLog(log);
  ...
  it("Should approve tokens for delegated transfers", async function () {
    ...
    const allowance = await glx.allowance(
      await account1.getAddress(),
      await account2.getAddress()
    );
  ...
  it("Should return the correct allowance", async function () {
    ...
    const allowanceAccount1 = await glx.allowance(
      await account1.getAddress(),
      await account2.getAddress()
    );
    const allowanceAccount2 = await glx.allowance(
      await account1.getAddress(),
      await account2.getAddress()
    );
  ...
  it("Should delegate votes to a delegatee", async function () {
    // Mint some tokens to account1
    const amountToMint = ethers.utils.parseEther("1000");
    await glx.connect(admin).mint(await account1.getAddress(), amountToMint);

    // Account1 delegates votes to account2
    await glx.connect(account1).delegate(await account2.getAddress());

    // Check the delegatee of account1
    const delegateeOfAccount1 = await glx.delegates(
      await account1.getAddress()
    );
  ...
  it("Should delegate votes by signature", async function () {
    ...
    const nonce = await glx.nonces(await account1.getAddress());
    const expiry = Math.floor(Date.now() / 1000) + 3600; // 1 hour from now

    // Create the EIP-712 domain separator
    const DOMAIN_TYPEHASH = await glx.DOMAIN_TYPEHASH();
    const domainSeparator = await glx.DOMAIN_SEPARATOR();

    // Create the EIP-712 delegation struct hash
    const DELEGATION_TYPEHASH = await glx.DELEGATION_TYPEHASH();
  ...
  it("Should get the current votes balance for an account", async function () {
    ...
    const currentVotes = await glx.getCurrentVotes(await account1.getAddress());
  ...
  it("Should determine the prior number of votes for an account", async function () {
    ...
    const priorVotes = await glx.getPriorVotes(
      await account1.getAddress(),
      blockNumber - 1
    );
  ...
  it("Should mint new tokens by the minter", async function () {
    ...
    const amountToMint = ethers.utils.parseEther("1000");
    await glx.connect(minter).mint(await recipient.getAddress(), amountToMint);

    // Verify that the recipient's balance is updated correctly
    const recipientBalance = await glx.balanceOf(await recipient.getAddress());
  ...
  it("Should not allow token minting by non-minter", async function () {
    ...
    const amountToMint = ethers.utils.parseEther("1000");
    try {
      await glx.connect(account2).mint(await recipient.getAddress(), amountToMint);
  ...
  it("Should update admin address", async function () {
    ...
    await glx.connect(admin).setAdmin(newAdminAddress);
  
    // Verify that the admin address is updated correctly
    const currentAdminAddress = await glx.admin();
  ...
  it("Should update minter address", async function () {
    ...
    await glx.connect(admin).setMinter(newMinterAddress);
  
    // Verify that the minter address is updated correctly
    const currentMinterAddress = await glx.minter();
  ...
  it("Should update stake modifier address", async function () {
    ...
    await glx.connect(admin).setStakeModifier(newStakeModifierAddress);
  
    // Verify that the stake modifier address is updated correctly
    const currentStakeModifierAddress = await glx.stakeModifier();
